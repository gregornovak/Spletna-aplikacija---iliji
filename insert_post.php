<?php
    ...
    if (!empty($title) && !empty($content)) {
        $query = mysqli_real_escape_string($link, "INSERT INTO blog(blog_title, blog_content) VALUES ('$title', '$content')");
        $result1 = mysqli_query($link, $query);
        //http://php.net/manual/en/mysqli.insert-id.php
        //shrani se zadnji id, ki ga nato lahko uporabim za zapis
        $blog_id = mysqli_insert_id($link);
        $query = mysqli_real_escape_string($link, "INSERT INTO blog_users (id_users, id_blog) VALUES ('$user_id', '$blog_id')");
